...
import { addExpense, deleteExpense, updateExpense } from '../redux/reducers/reducer';
  ...
  const [title, setTitle] = useState("");
    ...
    const handleAdd = () => {
        ...
        if (type !== "thu" && type !== "chi") {
           Alert.alert('Loại chỉ được nhập "thu" hoặc "chi')
            return;
        }
                ...
                <View key={row.id} style={styles.expenseContainer}>
                    {
                        (idEdit === row.id) ?
                            ...
                            (<>
                                <Text style={{ fontSize: 30, color: 'cyan', fontWeight: 'bold', alignSelf: 'center' }}>{row.title}</Text>
